...
typedef struct janus_streaming_rtp_source_stream {
    ...
    char source_ip[INET6_ADDRSTRLEN]; /* Store IPv4 or IPv6 address as string */
    janus_mutex source_ip_mutex;      /* Mutex to protect source_ip access */
	char *host;
...
static void janus_streaming_rtp_source_stream_free(const janus_refcount *st_ref) {
	...
	g_free(stream->mid);
	g_free(stream->label);
    janus_mutex_destroy(&stream->source_ip_mutex);
    janus_mutex_destroy(&stream->source_ip_mutex);
	g_free(stream->msid);
	g_free(stream->mstid);
...
janus_streaming_rtp_source_stream *janus_streaming_create_rtp_source_stream(
    ...
    stream->source_ip[0] = '\0';
janus_mutex_init(&stream->source_ip_mutex);
	stream->type = mtype;
