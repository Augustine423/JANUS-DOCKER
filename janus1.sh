...
# Function to check if a Unix socket path is in use
check_unix_socket() {
    local path=$1
    if [ -S "$path" ]; then
        echo "Error: Unix socket $path is already in use."
        exit 1
    fi
}
...
# Step 9: Apply custom video room plugin configuration
...
# Step 10: Apply custom Unix sockets transport configuration
...
# Step 11: Apply custom WebSockets transport configuration
...
# Step 12: Apply custom streaming plugin configuration
...
# Step 15: Stop any existing Janus processes
echo "Stopping any existing Janus processes..."
sudo pkill -f janus || true
sleep 2

# Step 16: Test Janus with NAT setting
...
# Step 17: Create systemd service for Janus
